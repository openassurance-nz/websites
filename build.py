...
def htaccess(domain, held_aliases=()):
  ...
  # E=NOCACHE marks this response so the header block below forbids caching
  # it. Without that, mod_expires applies the text/html rule to the redirect
  # and browsers replay it for an hour — which both hides any correction made
  # here and defeats the point of using a temporary redirect at all.
  RewriteCond %%{HTTP_HOST} ^(www\\.)?(%(names)s)\\.nz$ [NC]
  RewriteRule ^ https://%(domain)s/#profiles [L,NE,R=302,E=NOCACHE:1]
  # --- end TEMPORARY -----------------------------------------------------
...
<IfModule mod_headers.c>
  ...
  # Temporary redirects must never be cached. See E=NOCACHE above.
...
<IfModule mod_expires.c>
  ...
  # Ten minutes, not an hour. These are single-page sites under active
  # revision, and a long HTML cache means a correction is invisible to anyone
  # who has already visited until it expires.
